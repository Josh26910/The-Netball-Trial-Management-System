...
Added since last commit:
    - new Export tab: exports the current trial's player list to a clean
      CSV file (export.py) - no more merged cells or dash placeholders
      like the old spreadsheet system
"""
...
class App(tk.Tk):
    ...
    def build_export_tab(self):
        ...
        ttk.Button(
            self.export_tab, text="Export Players to CSV", command=self.export_players_csv
        ).pack(anchor="w", padx=10)
    ...
    def export_players_csv(self):
        ...
        export.export_players_csv(players, filepath)
